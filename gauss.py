...
def denoise_block(block, kernel_size):
    return cv2.GaussianBlur(block, (kernel_size, kernel_size), 0)

def divide_and_conquer_denoise(image, block_size, kernel_size):
    height, width = image.shape[:2]
    denoised_image = np.zeros_like(image)
    for i in range(0, height, block_size):
        for j in range(0, width, block_size):
            block = image[i:i + block_size, j:j + block_size]
            denoised_block = denoise_block(block, kernel_size)
            denoised_image[i:i + block_size, j:j + block_size] = denoised_block
    return denoised_image

def decrease_and_conquer_denoise(image, scale_factor, kernel_size):
    # Decrease the image size
    small_img = cv2.resize(image, (0, 0), fx=scale_factor, fy=scale_factor)
    
    # Denoise the smaller image
    denoised_small_img = cv2.GaussianBlur(small_img, (kernel_size, kernel_size), 0)
    
    # Increase the image size back to original
    enlarged_denoised_img = cv2.resize(denoised_small_img, (image.shape[1], image.shape[0]))
    
    return enlarged_denoised_img
...
block_size = 50
kernel_size = 5
denoised_img_divide_and_conquer = divide_and_conquer_denoise(gray_noisy_img, block_size, kernel_size)

# Menghilangkan noise dengan metode decrease and conquer
scale_factor = 0.5
denoised_img_decrease_and_conquer = decrease_and_conquer_denoise(gray_noisy_img, scale_factor, kernel_size)
denoised_img_gaussian = cv2.GaussianBlur(gray_noisy_img, (5, 5), 0)
